...
export const getGameResult = (): string | null => 
  store.getState().game.result;

export const getCurrentLevel = (): number | null => 
  store.getState().game.currentLevel;

export const getFinalWin = (): number => 
  store.getState().game.finalWin;

// --- MULTIPLIERS SELECTORS ---
export const getMultipliers = (): Record<string, number> => 
  store.getState().multipliers.values;
